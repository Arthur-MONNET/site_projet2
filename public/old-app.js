...
downloadButton.addEventListener("click", function () {
    ...
    console.info(blob)
    if (PHOTOID) {
        const file = new File([blob], 'audio_recording.wav', {type: blob.type})
...
inputsFile.forEach(input => {
    ...
    if (input.name === "image") {
        console.log("image")
        input.addEventListener("change", e => handlePhoto(e))
    } else {
        console.log("audio")
        input.addEventListener("change", e => handleAudio(e, input.name))
    }
});

const handleAudio = (event, type) => {
    if (PHOTOID) {
        ...
        formData.append('dataFile', files[0], files[0].name)
        formData.append('photoID', PHOTOID)
        formData.append('type', type)
        fetch('http://localhost:4000/upload-audio', {
            method: 'POST',
